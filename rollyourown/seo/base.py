...
class FormattedMetadata(object):
    ...
    def __init__(self, metadata, instances, path, site=None, language=None):
        ...
        if metadata._meta.use_cache:
            if metadata._meta.use_sites and site:
                hexpath = hashlib.md5(iri_to_uri(
                    site.domain + path)).hexdigest()
            else:
    ...
    def __getattr__(self, name):
        ...
        if name in self.__metadata._meta.groups:
            ...
            value = '\n'.join(unicode(BoundMetadataField(self.__metadata._meta.elements[
                              f], self._resolve_value(f))) for f in self.__metadata._meta.groups[name]).strip()
    ...
    def __unicode__(self):
        ...
        if value is None:
            value = mark_safe(u'\n'.join(unicode(getattr(self, f))
                                         for f, e in self.__metadata._meta.elements.items() if e.head))
            if self.__cache_prefix is not None:
...
class MetadataBase(type):
    ...
    def __new__(cls, name, bases, attrs):
        ...
        # Check that the names of the elements are not going to clash with a
        # model field
        for key in elements:
        ...
        try:
            ...
            raise Exception(
                'Metadata backend "%s" is not installed.' % backend_name)
    ...
    # TODO: Move this function out of the way (subclasses will want to define
...
def _get_metadata_model(name=None):
    ...
    if name is not None:
        try:
            ...
            if len(registry) == 1:
                valid_names = u'Try using the name "%s" or simply leaving it out altogether.' % registry.keys()[
                    0]
            else:
                valid_names = u"Valid names are " + \
                    u", ".join(u'"%s"' % k for k in registry.keys())
            raise Exception(u"Metadata definition with name \"%s\" does not exist.\n%s" % (
                name, valid_names))
    else:
        assert len(
            registry) == 1, "You must have exactly one Metadata class, if using get_metadata() without a 'name' parameter."
...
def _delete_callback(model_class, sender, instance,  **kwargs):
    ...
    model_class.objects.filter(
        _content_type=content_type, _object_id=instance.pk).delete()
...
def register_signals():
    for metadata_class in registry.values():
        ...
        if model_instance is not None:
            update_callback = curry(
                _update_callback, model_class=model_instance)
            delete_callback = curry(
                _delete_callback, model_class=model_instance)

            # Connect the models listed in settings to the update callback.
            for model in metadata_class._meta.seo_models:
                models.signals.post_save.connect(
                    update_callback, sender=model, weak=False)
                models.signals.pre_delete.connect(
                    delete_callback, sender=model, weak=False)
